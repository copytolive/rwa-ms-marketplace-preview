...
qsa('.asset-card').forEach(card=>{const heart=card.querySelector('.heart');if(!heart)return;const key=card.dataset.key;if(favorites.has(key)){heart.classList.add('liked');const i=heart.querySelector('i');if(i)i.className='fa-solid fa-heart'}heart.addEventListener('click',()=>{setTimeout(()=>{heart.classList.contains('liked')?favorites.add(key):favorites.delete(key);persistFavorites()},0)})});
const nav=qsa('.bottomnav button'),sheet=qs('#assetSheet');
if(nav[4]&&sheet){nav[4].addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();nav.forEach(x=>x.classList.remove('active'));nav[4].classList.add('active');sheet.dataset.mode='account';const selectedKey=localStorage.getItem('rwaSelectedAsset')||'';const selectedCard=selectedKey?qs(`.asset-card[data-key="${selectedKey}"]`):null;sheet.querySelector('img').src='assets/logo-ref-exact.png';sheet.querySelector('img').alt='RWA.ms';sheet.querySelector('.asset-sheet__eyebrow').textContent='Local Portfolio';sheet.querySelector('h3').textContent='Your RWA.ms workspace';sheet.querySelector('.asset-sheet__brand').textContent='Selections and favorites are saved on this device.';sheet.querySelector('.asset-sheet__price').textContent=selectedCard?'1 selected asset':'No asset selected';sheet.querySelector('[data-fact="holders"]').textContent=`${favorites.size} saved`;sheet.querySelector('[data-fact="sold"]').textContent=selectedCard?'Ready to review':'Browse marketplace';const cta=sheet.querySelector('.asset-sheet__cta');cta.textContent=selectedCard?'View selected asset':'Browse assets';sheet.classList.add('open');document.body.style.overflow='hidden'},true);
const cta=sheet.querySelector('.asset-sheet__cta');cta.addEventListener('click',e=>{if(sheet.dataset.mode!=='account')return;e.preventDefault();e.stopImmediatePropagation();const selectedKey=localStorage.getItem('rwaSelectedAsset')||'';sheet.classList.remove('open');sheet.dataset.mode='';document.body.style.overflow='';if(selectedKey){qs(`.asset-card[data-key="${selectedKey}"]`)?.click()}else{qsa('.m-tabs button')[0]?.click();qs('.m-featured')?.scrollIntoView({behavior:'smooth',block:'start'})}},true);
sheet.querySelector('.asset-sheet__close')?.addEventListener('click',()=>{sheet.dataset.mode=''});sheet.querySelector('.asset-sheet__backdrop')?.addEventListener('click',()=>{sheet.dataset.mode=''})}
const chartSeries={
...
const targets=[qs('.m-hero'),qs('.m-featured'),qs('.m-featured'),qs('.m-featured')];
const activate=i=>nav.forEach((b,n)=>b.classList.toggle('active',n===i));
if('IntersectionObserver' in window){const spy=new IntersectionObserver(es=>{const hit=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(!hit)return;const key=hit.target.dataset.spy;if(key==='home')activate(0);if(key==='marketplace')activate(1)}, {threshold:[.25,.5],rootMargin:'-18% 0px -45%'});if(targets[0]){targets[0].dataset.spy='home';spy.observe(targets[0])}if(targets[1]){targets[1].dataset.spy='marketplace';spy.observe(targets[1])}}
