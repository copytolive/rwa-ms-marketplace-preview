...
function bindCards(){qsa('.asset-grid .heart').forEach(b=>b.addEventListener('click',()=>{b.classList.toggle('liked');b.querySelector('i').className=b.classList.contains('liked')?'fa-solid fa-heart':'fa-regular fa-heart'}));qsa('.asset-grid .detail').forEach(b=>b.addEventListener('click',()=>{const key=b.closest('.asset-card')?.dataset.key;if(key)try{window.top.location.href='asset-detail.html?asset='+encodeURIComponent(key)}catch{location.href='asset-detail.html?asset='+encodeURIComponent(key)}}));}
